#!/usr/bin/env node
/**
 * Send the E Lion Record Label (mainstream) campaign to contacts/elion-record-label-mainstream-50.csv.
 *
 * Usage (same as levelup):
 *   SMOOTHSALES_URL=https://smoothsales-app.vercel.app node scripts/send-record-label-mainstream.js
 *   node scripts/send-record-label-mainstream.js https://smoothsales-app.vercel.app
 *   node scripts/send-record-label-mainstream.js https://smoothsales-app.vercel.app contacts/other-mainstream.csv
 *
 * Requires: App deployed with elion-record-label-mainstream in validIds; RESEND_API_KEY set in Vercel.
 */
...
const csvPath = csvArg
  ...
  : path.join(__dirname, "..", "contacts", "elion-record-label-mainstream-50.csv");
...
console.log(`Sending elion-record-label-mainstream to ${recipients.length} recipients via ${baseUrl}`);

async function run() {
  const res = await fetch(`${baseUrl.replace(/\/$/, "")}/api/send-campaign`, {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify({ templateId: "elion-record-label-mainstream", recipients }),
  });
