...
import {
  ...
  clearAskHtml,
  clearAskText,
  credibilityProofHtml,
  credibilityProofText,
} from "./head-hunt-credibility";
import {
  card,
  fbox,
  replyCtaButtons,
...
function proofFor(slug: string) {
  const focus = PROOF_FOCUS_BY_SLUG[slug] ?? "business";
  const ask =
    ASK_BY_SLUG[slug] ??
    "Would you be open to a brief conversation on Oahu or by phone?";
  return { focus, ask };
}

function buildInitial(a: Audience) {
  const { focus, ask } = proofFor(a.slug);
  const cta = replyCtaButtons(a.accent, a.slug);
  const proofHtml = credibilityProofHtml(focus, a.accent);
  const proofText = credibilityProofText(focus);
  const askHtml = clearAskHtml(ask, a.accent);
  const askText = clearAskText(ask);
  const text = `Hi {{Name}},\n\n${a.textBody}${proofText}${askText}${MEETING_TEXT}${REPLY_CTA_TEXT}${FOOTER_TEXT}`;
  const html = card(
    a.accent,
    a.accent + "33",
    a.kicker,
    a.headline,
    a.sub,
    `${a.htmlBody}${proofHtml}${askHtml}${MEETING_HTML}${cta}`,
    FOOTER_HTML,
  );
...
function buildFollowUp1(a: Audience) {
  ...
  const askHtml = clearAskHtml(ask, a.accent);
  const askText = clearAskText(ask);
  const text = `Hi {{Name}},\n\nQuick bump — my note may have landed on a busy day. I am still looking for someone passionate about this lane (${a.label}). If it is not you, a one-line point-in-the-right-direction would mean a lot.${askText}${REPLY_CTA_TEXT}\n\nEric${FOOTER_TEXT}`;
  const html = fbox(
    a.accent,
...
<p style="margin:0 0 12px;font-size:14px;line-height:1.7;">If it is not you, a one-line referral helps tremendously.</p>
${credibilityProofHtml(focus, a.accent)}${askHtml}${cta}`,
    FOOTER_HTML,
  );
